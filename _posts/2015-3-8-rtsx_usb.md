...
I ignored MFD<sub>RTSX</sub><sub>PCI</sub> and enabled
MFD<sub>RTSX</sub><sub>USB</sub> from Device Drivers ->
...
Again I safely ignored MMC<sub>REALTEK</sub><sub>USB</sub> and enabled
MMC<sub>REALTEK</sub><sub>USB</sub> from Device Drivers -> MMC/SD/SDIO
card support -> Realtek USB SD/MMC Card interface Driver as module.
...
__PS__: The module `rtsx_usb` should load automatically. If the
module is not able to load, create a file rtsx<sub>usb</sub>.conf in
the path /etc/modules-load.d with following content.
